      ...
      >
        ...
        <DialogContent>
          <div >
            ...
            <TextField id="outlined-basic" style={{ marginBottom: '18px' }} defaultValue="Phone" value={phoneSearch} onChange={(e)=>setPhoneSearch(e.target.value)}  fullWidth label="Phone" variant="outlined" />         
          </div>
        </DialogContent>
        <DialogActions>
      ...
      >
        <Container maxWidth={false}>
          ...
          >
            ...
            {(nameSearch||searchBarName||idSearch||countrySearch||linkedinSearch ||companySearch||desigSearch||emailSearch || phoneSearch)?(<Typography variant="subtitle1" style={{margin:"0px 20px 5px 0px"}}>Total results found {usersList.users.filter((elem)=>((elem.first_name?(elem.first_name.includes(nameSearch)):(nameSearch?false:true)) ||(elem.last_name?(elem.last_name.includes(nameSearch)):(nameSearch?false:true))) && ((elem.first_name?(elem.first_name.includes(searchBarName)):(searchBarName?false:true)) || (elem.first_name?(elem.first_name.includes(searchBarName)):(searchBarName?false:true)) || (elem && elem.emails && elem.emails.length>0 ? (elem.emails[0].address.includes(searchBarName)):(searchBarName?false:true))) && (elem.linkedin_username?(elem.linkedin_username.includes(linkedinSearch)):(linkedinSearch?false:true)) && (elem.location_country?(elem.location_country.includes(countrySearch)):(countrySearch?false:true)) && (elem.job_company_name?(elem.job_company_name.includes(companySearch)):(companySearch?false:true)) && (elem.job_title?(elem.job_title.includes(desigSearch)):(desigSearch?false:true)) && (elem && elem.emails && elem.emails.length>0 ? (elem.emails[0].address.includes(emailSearch)):(emailSearch?false:true)) && (elem.mobile_phone ? (elem.mobile_phone.includes(phoneSearch)):(phoneSearch?false:true))).length} </Typography>):(null)}
            {(nameSearch||idSearch||countrySearch||linkedinSearch ||companySearch||desigSearch||emailSearch || phoneSearch)?(<Button type="text" style={{marginRight:"10px"}} onClick={()=>{setNameSearch(""); setLinkedinSearch(""); setEmailSearch(""); setPhoneSearch(""); setCompanySearch(""); setDesigSearch(""); setCountrySearch("");}}> Clear Filters</Button>):(null)}
          ...
          >
            Filter ({(nameSearch?1:0)+(linkedinSearch?1:0)+(companySearch?1:0)+(desigSearch?1:0)+(emailSearch?1:0)+(idSearch?1:0)+(phoneSearch?1:0)})
          </Button>
          </div>
          ...
          <Table >
              ...
              {usersList.users.length>0?(<>
                {usersList.users.filter((elem)=>((elem.first_name?(elem.first_name.includes(nameSearch)):(nameSearch?false:true)) ||(elem.last_name?(elem.last_name.includes(nameSearch)):(nameSearch?false:true))) && ((elem.first_name?(elem.first_name.includes(searchBarName)):(searchBarName?false:true)) || (elem.first_name?(elem.first_name.includes(searchBarName)):(searchBarName?false:true)) || (elem && elem.emails && elem.emails.length>0 ? (elem.emails[0].address.includes(searchBarName)):(searchBarName?false:true))) && (elem.linkedin_username?(elem.linkedin_username.includes(linkedinSearch)):(linkedinSearch?false:true)) && (elem.location_country?(elem.location_country.includes(countrySearch)):(countrySearch?false:true)) && (elem.job_company_name?(elem.job_company_name.includes(companySearch)):(companySearch?false:true)) && (elem.job_title?(elem.job_title.includes(desigSearch)):(desigSearch?false:true)) && (elem && elem.emails && elem.emails.length>0 ? (elem.emails[0].address.includes(emailSearch)):(emailSearch?false:true)) && (elem.mobile_phone ? (elem.mobile_phone.includes(phoneSearch)):(phoneSearch?false:true)) ).slice(start,end).map((user)=><TableRow> 
                    ...
                    <TableCell>{user && user.phone_numbers && user.phone_numbers.length>0 ?(<>{user.phone_numbers[0]}</>):(<>{user.mobile_phone?user.mobile_phone:"N/A"}</>)}</TableCell>
